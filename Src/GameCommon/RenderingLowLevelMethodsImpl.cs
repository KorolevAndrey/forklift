...
namespace GameCommon
{
	...
	// Usually this class need to change only for optimizations and for advanced features.
	//
	// Bad idea to realize here changing visibility of game objects.
	// Use MapObject.OnRender() and SceneNode.Visible for this.
	//
	// If you want move this class to different assembly, you need update 
	// "lowLevelMethodsAssemblyName" property in the "Definitions\Renderer.config".
	//
	public class RenderingLowLevelMethodsImpl : RenderingLowLevelMethods
	{
		...
		ConvexPolyhedron GetConvexPolyhedronFromFrustum( ref Frustum frustum )
		{
			...
			return new ConvexPolyhedron( points, faces );
		}
		...
		static ConvexPolyhedron MakeConvexPolyhedronForPointLight( RenderLight light )
		{
			...
			Vec3[] vertices;
			int[] indices;
			GeometryGenerator.GenerateSphere( radius, 8, 8, false, out vertices,
				out indices );

			for( int n = 0; n < vertices.Length; n++ )
				vertices[ n ] = light.Position + vertices[ n ];

			return new ConvexPolyhedron( vertices, indices );
		}
		...
		static ConvexPolyhedron MakeConvexPolyhedronForSpotLight( RenderLight light )
		{
			...
			return new ConvexPolyhedron( vertices.ToArray(), faces.ToArray() );
		}
		...
		static List<Plane> GetClipPlanesOfConvexHullAroundMainCameraAndLightPosition(
			Camera mainCamera, RenderLight light )
		{
			...
			return new List<Plane>( hullPlanes );
		}
			...
			List<SceneNode> outSceneNodes, List<StaticMeshObject> outStaticMeshObjects )
		{
			...
			clipPlanes.AddRange( light.SpotLightClipPlanes );

			//add main frustum clip planes and light position
			{
				List<Plane> hullPlanes = GetClipPlanesOfConvexHullAroundMainCameraAndLightPosition(
					mainCamera, light );
				clipPlanes.AddRange( hullPlanes );
			}

			Sphere clipSphere = new Sphere( light.Position, attenuationRange );

			SceneManager.Instance._SceneGraph.GetObjects( clipPlanes.ToArray(),
				...
				delegate( _ISceneObject sceneObject )
				{
					...
					if( ( objGroups & _SceneObjectGroups.SceneNode ) != 0 )
					{
						...
						bool existsCastsShadows = false;
						for( int n = 0; n < sceneNode.MovableObjects.Count; n++ )
						{
							if( sceneNode.MovableObjects[ n ].CastShadows )
								existsCastsShadows = true;
						}
						if( !existsCastsShadows )
							return;
					...
					Log.Fatal( "MyRenderingLowLevelMethods: WalkSpotLightShadowGeneration: invalid sceneObject." );
				} );
		}
			...
			List<StaticMeshObject> outStaticMeshObjects )
		{
			...
			if( !pointLightFaceDirection.Equals( Vec3.Zero, .001f ) )
			{
				...
				{
					Plane[] lightClipPlanes = GetClipPlanesForPointLightShadowGeneration( mainCamera,
						light, pointLightFaceDirection );
					clipPlanes.AddRange( lightClipPlanes );
				}
				...
				SceneManager.Instance._SceneGraph.GetObjects( clipPlanes.ToArray(),
					...
					delegate( _ISceneObject sceneObject )
					{
						...
						if( ( objGroups & _SceneObjectGroups.StaticMeshObject ) != 0 )
						{
							...
							//clip by sphere
							//possible optimization: faster check by bounding sphere
							if( !clipSphere.IsIntersectsBounds( sceneObject._SceneData.Bounds ) )
								return;
						...
						Log.Fatal( "MyRenderingLowLevelMethods: WalkPointLightShadowGeneration: invalid sceneObject." );
					} );
			}
			else
			{
				...
				Sphere lightSphere = new Sphere( light.Position, attenuationRange );

				List<Plane> clipPlanes = new List<Plane>();

				//add 6 light clip planes.
				{
					clipPlanes.Add( new Plane( Vec3.XAxis, lightSphere.Origin.X + lightSphere.Radius ) );
					clipPlanes.Add( new Plane( -Vec3.XAxis, -( lightSphere.Origin.X - lightSphere.Radius ) ) );
					clipPlanes.Add( new Plane( Vec3.YAxis, lightSphere.Origin.Y + lightSphere.Radius ) );
					clipPlanes.Add( new Plane( -Vec3.YAxis, -( lightSphere.Origin.Y - lightSphere.Radius ) ) );
					clipPlanes.Add( new Plane( Vec3.ZAxis, lightSphere.Origin.Z + lightSphere.Radius ) );
					clipPlanes.Add( new Plane( -Vec3.ZAxis, -( lightSphere.Origin.Z - lightSphere.Radius ) ) );
				}
				...
				SceneManager.Instance._SceneGraph.GetObjects( clipPlanes.ToArray(),
					//SceneManager.Instance._SceneGraph.GetObjects( ref clipSphere,
					_SceneObjectGroups.SceneNode | _SceneObjectGroups.StaticMeshObject,
					delegate( _ISceneObject sceneObject )
					{
						_SceneObjectGroups objGroups = sceneObject._SceneData.Groups;
						...
						if( ( objGroups & _SceneObjectGroups.StaticMeshObject ) != 0 )
						{
							...
							//clip by sphere
							//possible optimization: faster check by bounding sphere
							if( !lightSphere.IsIntersectsBounds( sceneObject._SceneData.Bounds ) )
								return;
		...
		static List<Plane> GetClipPlanesForDirectionalLightShadowGeneration( Camera mainCamera,
			RenderLight light, int pssmTextureIndex )
		{
...

			{
				...
				int[] edges = ConvexPolygon.GetFromPoints( projectedFrustumPoints );
		...
		{
			List<Plane> clipPlanes = GetClipPlanesForDirectionalLightShadowGeneration( mainCamera,
				light, pssmTextureIndex );
			...
			SceneManager.Instance._SceneGraph.GetObjects( clipPlanes.ToArray(),
				...
				delegate( _ISceneObject sceneObject )
				{
					...
					Log.Fatal( "MyRenderingLowLevelMethods: WalkDirectionalLightShadowGeneration: invalid sceneObject." );
				} );
		}

		//you can use this method for debugging purposes.
		static void WalkAll( List<SceneNode> outSceneNodes,
			List<StaticMeshObject> outStaticMeshObjects )
		{
			...
			List<SceneNode> outSceneNodes, List<StaticMeshObject> outStaticMeshObjects )
		{
			...
			SceneManager.Instance._SceneGraph.GetObjects( ref frustum, false,
				...
				delegate( _ISceneObject sceneObject )
				{
					...
					Log.Fatal( "MyRenderingLowLevelMethods: OnSceneManagementGetObjectsForCamera: invalid sceneObject." );
				} );
